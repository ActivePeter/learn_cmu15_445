...
IndexPageType BPlusTreePage::GetPageType(){
    return page_type_;
}
...
void BPlusTreePage::IncreaseSize(int amount) {
    if(IsLeafPage()){
        if(size_+amount>GetMaxSize()+1){
            throw Exception(ExceptionType::OUT_OF_RANGE,"IncreaseSize");
        }
    }else{
        if(size_+amount>GetMaxSize()+2){
            throw Exception(ExceptionType::OUT_OF_RANGE,"IncreaseSize");
        }
    }
    size_+=amount;
